...
var rootPath = process.env.METHODE_API_ROOTPATH
...
var jar = request.jar(new tcf(cookiepath))
    , request = request.defaults({headers: {'SWTToken': process.env.METHODE_SWTTOKEN}, jar: jar})
    , retry = 0;
...
function handleResponse(res, data, filename, cb) {
    ...
    if (res.statusCode === 401 && ++retry < 3) {
        ...
        cb('File successfully uploaded ' + uploadPath + '/' + constructFileName(filename))
    }
    else {
...
function authenticateAndUpload(data, filename, cb) {
    request.post(rootPath + '/auth/login', {
        form: {
            username: process.env.METHODE_USERNAME,
            pwd: process.env.METHODE_PASSWORD,
            connectionId: process.env.METHODE_CONNECTIONID
        }
    }, function () {
